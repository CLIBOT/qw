...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "کاربر یا یوزرنیم مورد نظر پیدا نشد!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "شما نمیتوانید مدیران/مالکان/ادمین هارا اخراج کنید")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "شما نمیتوانید مدیران/مالکان/ادمین هارا بن کنید")
			return
        end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] انبن گلوبال شد')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ایدی' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == 'اخراجم_کن' and msg.to.type == "chat" then-- /kickme
  ...
  if matches[1]:lower() == "لیست_بن" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'بن' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید مدیران/مالکان/ادمین هارا بن کنید"
        end
...
if matches[1]:lower() == 'اخراج' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید مدیران/مالکان/ادمین هارا اخراج کنید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما نمیتوانید خود را اخراج کنید !"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
  ...
  if matches[1]:lower() == 'بن_ال' and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'انبن_ال' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] انبن گلوبال شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "لیست_بن_گلوبال" then -- Global ban list
    return banall_list()
  end
end
